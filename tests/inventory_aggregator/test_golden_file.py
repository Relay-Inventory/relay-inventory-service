...
def test_golden_output_matches_expected() -> None:
    ...
    rows = priced.to_dict(orient="records")
    ...
    # Snapshot contract per IMPLEMENTATION_PLAN.md Sec 5: combined availability + cheapest
    # source, replacing the old single-winner-per-SKU shape (which carried per-vendor fields
    # like quantity_available/vendor_id/updated_at directly).
    output_columns = ["sku", "available_qty", "source_vendor_id", "source_cost", "vendor_count", "price"]
